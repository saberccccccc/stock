...
print(f"  - 聚合特征: {agg_dim}缁?({base_feat_dim}涓?熀纭EUR特征 × {N_AGGS}绉嶈仛鍚?")
print(f"  - Rank特征: {rank_dim}缁?(鎴?潰鎺掑簭)")
...
print(f"  - 市场特征: {market_feat_dim}缁?(16宽基+3宽度+31行业收益+31琛屼笟鍙?敤鎬?")
print(f"  - 瀹忚?/璧勯噾娴? {macro_feat_dim}缁?(north, margin, PMI)")
print(f"  - 行业one-hot: {industry_onehot_dim}缁?")
print(f"  - 行业embedding: {num_industries}涓?湡瀹炶?涓?+ 1涓?湭鐭ヨ?涓?")
...
if device.type == "cuda":
    ...
    if gpu_mem < 8:
        batch_size, accum_steps = 2, 8      # 等效 batch 16
    else:
