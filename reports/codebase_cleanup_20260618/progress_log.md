...
## Phase 18 Archive/Cache Cleanup

### Completed

Archived the four old archive/cache candidates:

```text
.pytest_cache
__pycache__
_archive_models_data_20260604
_archive_results_20260604
```
...
The two volatile cache directories, `.pytest_cache` and `__pycache__`, can be
regenerated by tests/imports. To avoid endless cleanup churn, source inventory
now skips those volatile top-level caches. The older `_archive_*` payloads were
moved out of the top-level project root.

Current summary:

```text
top-level inventory rows=105
archive_candidate=66
protect=18
review=21
archive_or_cache candidates=0
checkpoint_or_model candidates=30
experiment_output candidates=36
```
...
Only two cleanup classes remain: referenced/high-risk checkpoint/model outputs
