...
- [x] Create a no-hardlink standalone Git clone and install its metadata in
  `deepseek_model_exp`.
- [x] Verify branches, objects, remote, status, and tests from the standalone
  repository.
- [x] Remove the retired legacy worktree only after all gates pass.
