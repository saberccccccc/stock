...
```text
PROJECT_RULES.md
-> MASTER_QUANT_RESEARCH_EXECUTION_PLAN_20260718.md
-> .planning/.active_plan 指向的唯一执行单
-> 当前阶段的一份详细技术说明（可选）
```

- 本文件是唯一决定 P0-P9 顺序和跨阶段门禁的计划。
- `.planning/.active_plan` 同一时间只能指向一个执行单；其他 `.planning` 目录都是
  已完成、暂停或移交的历史证据。
- 当前阶段最多保留一份详细技术说明；它只能展开任务、产物和验收方式，不能改变总顺序。
- 旧 Qlib、Rolling、reranker、loss、验证和重构计划继续保留事实与历史结论，但其
  “下一步”文字全部失效。
- 新建计划前必须先判断能否直接追加到活动执行单；没有新的阶段边界、协议或独立门禁时，
  禁止再创建计划文件。
- 计划状态统一登记在 `.planning/README.md`；不靠文件名或修改时间判断当前计划。
...
## 14. 非训练闭环实施包（2026-07-19）

当新增训练暂停时，P4 的训练门不得被临时调参或跳阶段替代。仓库耐久化、正式基线冻结、
数据与执行审计、现有证据统一 replay、归因、回测性能等价优化和 prepared-only Shadow
演练，统一按 `NON_TRAINING_RESEARCH_CLOSURE_PLAN_20260719.md` 执行。

该实施包只补齐现有系统和证据，不改变 P0-P9 顺序，不以 2026 Forward 选择规则，
不启动训练，也不授权 lifecycle 从 `prepared` 转为 `shadow`。
