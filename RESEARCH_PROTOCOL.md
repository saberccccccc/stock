...
& $python data/update_daily.py --data-dir data/forward_raw
& $python scripts/update_forward_market_data.py --data-dir data/forward_raw
```

Generate the frozen forward Alpha only after both update commands succeed:

```powershell
& $python run/forward_frozen_strategy.py `
