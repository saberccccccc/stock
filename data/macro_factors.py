# macro_factors.py 鈥?瀹忚?涓庤祫閲戞祦鍥犲瓙鑾峰彇锛坅kshare鍏嶈垂鏁版嵁婧愶級
import os
import time
import random
import numpy as np
...
# ==================== 瀹夊叏璋冪敤锛堝弬鑰僒ushareProLite闄愭祦閫昏緫锛?===================
_last_request_time = 0
_min_interval = 1.5


def _safe_ak_call(func, *args, **kwargs):
    """甯﹂噸璇曞拰闄愭祦鐨刟kshare璋冪敤"""
    ...
    for attempt in range(3):
        try:
            ...
            print(f"  璇锋眰澶辫触 ({attempt+1}/3): {e}, 绛夊緟 {4*(attempt+1)}s")
            time.sleep(4 * (attempt + 1))
    return None

# ==================== 鍖楀悜璧勯噾鍑EUR娴佸叆 ====================
def fetch_north_flow(save_path=None):
    """鑾峰彇鍖楀悜璧勯噾鏃ラ?鍑EUR娴佸叆鏁版嵁"""
    ...
    if df is not None and not df.empty:
        ...
        result = pd.DataFrame({'net_flow': net.values}, index=df['鏃ユ湡'])

        if df_sz is not None and not df_sz.empty:
            df_sz['鏃ユ湡'] = pd.to_datetime(df_sz['鏃ユ湡'])
            net_sz = pd.to_numeric(df_sz['褰撴棩鎴愪氦鍑EUR涔伴?'], errors='coerce').fillna(0)
            sz_series = pd.Series(net_sz.values, index=df_sz['鏃ユ湡'])
...
def fetch_margin_balance(save_path=None):
    ...
    print("涓嬭浇铻嶈祫铻嶅埜鏁版嵁...")
    # 浣跨敤瀹忚?鎺ュ彛锛歮acro_china_market_margin_sh / sz
    ...
    for df, label in [(df_sh, 'sh'), (df_sz, 'sz')]:
        if df is not None and not df.empty and '鏃ユ湡' in df.columns and '铻嶈祫铻嶅埜浣欓?' in df.columns:
            df['鏃ユ湡'] = pd.to_datetime(df['鏃ユ湡'])
            daily = df.groupby('鏃ユ湡')['铻嶈祫铻嶅埜浣欓?'].last()
            frames.append(daily)
...
def fetch_pmi(save_path=None):
    ...
    if df is not None and not df.empty:
        ...
        df['month_date'] = df['鏈堜唤'].apply(parse_pmi_month)
        df['pmi'] = pd.to_numeric(df['鍒堕EUR犱笟-鎸囨暟'], errors='coerce')
...
def build_macro_features(all_dates):
    ...
    杩斿洖: DataFrame index=dates, columns=[north_net_zscore, margin_balance_change, pmi_zscore]
    """
