...
鐢ㄦ硶锛?
  ...
  python update_data.py --extend 20100101    # 鎵╁睍鍘嗗彶 + 鏇存柊鏈EURO鏂?
...
def resolve_tushare_token(token=None):
    ...
    if not resolved:
        raise ValueError('缂哄皯 TUSHARE_TOKEN锛岃?璁剧疆鐜??鍙橀噺鎴栭EURO氳繃 --token 浼犲叆')
    return resolved
...
class TushareProLite:
    ...
    def update_single_stock(self, ts_code, data_dir, start_date='20100101'):
        """鏅鸿兘澧為噺鏇存柊锛氳ˉ鍏ㄧ己澶辩殑鍘嗗彶 + 杩藉姞鏈EURO鏂版暟鎹?"""
        ...
        if not local.empty:
            ...
            # 鎯呭喌1锛氭湰鍦板凡瑕嗙洊鍏ㄩ儴鑼冨洿 鈫?鍙?ˉ鏈EURO鏂?
...
def batch_update(data_dir, token, start_date='20100101', max_workers=3,
    ...
    for start in range(0, total, batch_size):
        ...
        print(f"杩涘害: {start + len(batch)}/{total}  |  "
              f"鏇存柊:{updated} 璺宠繃:{skipped} 澶辫触:{failed}")
    ...
    print(f"\n{'='*60}")
    print(f"鏇存柊瀹屾垚! 鏇存柊:{updated} 璺宠繃:{skipped} 澶辫触:{failed}")
    print(f"{'='*60}")


# ==================== 鏇存柊瀹忚?鏁版嵁缂撳瓨 ====================
def update_macro_cache():
    """鍒犻櫎鏃х紦瀛橈紝閲嶆柊涓嬭浇瀹忚?鏁版嵁"""
    cache_dir = "cache"
...
def main():
    ...
    parser.add_argument('--start-date', type=str, default='20100101',
                        help='璧峰?鏃ユ湡 YYYYMMDD (榛樿?20100101)')
...
def fetch_index_data(symbol, save_path, start_date="2010-01-01", end_date="2026-12-31"):
    ...
    df.to_csv(save_path)
    print(f"鎸囨暟鏁版嵁宸蹭繚瀛樿嚦 {save_path}锛屽叡 {len(df)} 鏉?")
...
def fetch_industry_data(save_path="data/stock_industry.csv"):
    ...
    lg = bs.login()
    if lg.error_code != '0':
        print(f"鐧诲綍澶辫触: {lg.error_msg}")
        return
    rs = bs.query_stock_industry()
    if rs.error_code != '0':
        print(f"鏌ヨ?澶辫触: {rs.error_msg}")
        bs.logout()
        return
    industry_list = []
    while rs.next():
    ...
    bs.logout()
    print(f"琛屼笟鏁版嵁宸蹭繚瀛樿嚦 {save_path}锛屽叡 {len(result)} 鏉?")
